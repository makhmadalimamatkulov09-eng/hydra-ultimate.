...
LLM_APIS = {
    "pollinations": {"key": None, "url": "https://text.pollinations.ai/"},
...
def init_db():
    ...
    cur.execute('''CREATE TABLE IF NOT EXISTS jailbreaks
                 (id SERIAL PRIMARY KEY, prompt TEXT, source TEXT, success_rate REAL DEFAULT 0.0,
...
def add_jailbreak(prompt, source="manual"):
    ...
    conn = get_db_conn()
    cur = conn.cursor()
    cur.execute("INSERT INTO jailbreaks (prompt, source) VALUES (%s, %s)", (enc, source))
    conn.commit()
    cur.close()
    conn.close()
...
def get_top_jailbreaks(n=10):
    conn = get_db_conn()
    cur = conn.cursor()
    cur.execute("SELECT id, prompt, success_rate FROM jailbreaks ORDER BY success_rate DESC LIMIT %s", (n,))
    rows = cur.fetchall()
    cur.close()
    conn.close()
...
def update_jailbreak_stats(jb_id, shackle):
    conn = get_db_conn()
    cur = conn.cursor()
    cur.execute("UPDATE jailbreaks SET times_used = times_used + 1, success_rate = (success_rate * (times_used - 1) + %s) / times_used, last_used = NOW() WHERE id = %s", (shackle, jb_id))
    conn.commit()
    cur.close()
    conn.close()
...
async def async_query_llm(prompt, llm_name="pollinations", jb_id=None):
    ...
    headers = {"User-Agent": "Mozilla/5.0"} if llm_name == "pollinations" else \
    ...
    if llm_name == "pollinations":
        full = jb['prompt'] + "\nUser: " + prompt + "\nAssistant:"
        payload = {"prompt": full, "model": "openai"}
    else:
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if r.status_code == 200:
                data = r.json()
                text = data.get("text") or data["choices"][0]["message"]["content"]
